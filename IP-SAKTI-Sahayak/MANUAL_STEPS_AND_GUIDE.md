...
**Starter KB included.** `backend/data/pdfs/` now ships with 6 seed documents
(with sidecar `.json` metadata) covering patent TK/Section 3(d)/3(p)
guidance, trademark Section 9/11 objections, AYUSH GMP (Schedule T),
AYUSH labelling (Rule 161), TKDL/PCT national phase, and AYUSH startup
licensing. These were written from general legal knowledge for demo/bootstrap
purposes — **they are not verbatim official government text** and should be
treated as a starting point, not a substitute for the real filings. Run
`python scripts/ingest.py` once to index them (see §5) and the "Knowledge
base is empty" banner will clear.

For production use and high-precision citations, replace/extend these with
**15 to 25 official PDFs** downloaded directly from government portals rather
...
- **Action**: Drag and drop the sample notice PDF at `backend/data/samples/sample_trademark_exam_report.pdf`
  (a synthetic Section 9(1)(a)/11(1) objection notice generated for demo purposes — not a real IP India filing).
- **What to highlight to judges**:
