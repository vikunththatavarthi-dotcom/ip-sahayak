...
def _call_ollama(
    ...
    # httpx.Timeout: give more headroom to "read" (generation) than to connect,
    # since a slow/cold local model is a read-timeout problem, not a connect problem.
    ...
    attempts = max(1, settings.ollama_max_retries + 1)
    ...
    return (
        "[Error: The local model took too long to respond, even after a retry. "
        "This usually means the model is too large for this machine, another process "
        "is using the GPU/CPU, or Ollama had to reload the model from disk. Try: "
        "1) a smaller model (e.g. `ollama pull llama3.2:1b`), "
        "2) running `ollama run <model>` once beforehand to warm it up, "
        "3) lowering OLLAMA_NUM_CTX in .env, or "
        "4) checking `ollama ps` for stuck processes.]"
    )
