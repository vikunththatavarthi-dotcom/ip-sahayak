...
def _load_model():
    ...
    try:
        ...
        logger.info("Loading FastEmbed TextEmbedding model (%s)...", settings.embed_model)
...
def embedding_dimension() -> int:
    """Return the actual output dimension of the current embedding model."""
    try:
        vector = embed_text("warm up")
        return len(vector)
    except Exception:
        return 384
