...
class LocalFAISSVectorStore:
    ...
    def reset(self) -> None:
        """Clear the local index explicitly before a complete KB re-ingestion."""
        self.ids, self.documents, self.metadatas, self.index = [], [], [], None
        for path in (self.index_file, self.meta_file):
            if path.exists():
                path.unlink()
        logger.info("Cleared local FAISS knowledge index.")
...
def reset_collection() -> None:
    """Clear persisted vectors. Intended only for an explicit ingestion reset."""
    _get_store().reset()
